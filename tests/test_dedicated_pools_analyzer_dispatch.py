...
def test_config_auto_populates_dedicated_sql_scope(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    assert scope.display_name == "examplesqlserver"
    assert scope.resource_group == "rg-1"
    assert "Microsoft.Sql/servers/examplesqlserver" in scope.id
    assert scope.extras["sql_server_fqdn"] == "examplesqlserver.database.windows.net"
