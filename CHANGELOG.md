...
- **Slice F — Standalone Dedicated SQL pool monitoring (DWU
  ...
  - `MODULE_SPECS["monitoring"].supports` widened to include
    ...
    scopes. An existing scan output at
...
- **SQL Surface page renders for pools without code objects.**
  ...
  raw-staging DWUs) short-circuited to *"No code objects or
  serverless query history collected"* before the Top Queries /
  Top Consumed Objects / Workload Capture Stats sections had a
  chance to render. `hasDedicated` now also accepts those
  alternative signals; the Code Objects table is still
  conditionally hidden when its row set is empty so the page
  surface stays tidy.
